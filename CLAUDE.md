...
**The API spec at `.brainstorm/spec.md` is the contract between backend and iOS.** Both sides conform to it independently.
...
2. **Update the spec first.** Before changing any endpoint's request or response shape, update `.brainstorm/spec.md`. Then update the backend, then iOS.
...
1. Update `.brainstorm/spec.md` with the new request/response shape
